...
const updateFilteredEmgHistory = (
  filteredEmg: MyoModFilteredEmgData,
  rawCounter: number,
  history: FilteredEmgHistory[],
  packetCount: number | null
  ...
  history.push({
    timestamp: now,
    values: new Float32Array(filteredEmg.data),
...
function DownloadButton() {
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `emg-data-${new Date()
      .toISOString()
      .slice(0, 19)
      .replace(/:/g, "-")}.json`;
      ...
      >
        ...
        >
          <Suspense fallback={null}>
            <Hand myoMod={myoMod} updateFramerate={updateFramerate} />
          </Suspense>
        </group>
...
function Chart({ samplingRate }: { samplingRate: number }) {
  ...
  useEffect(() => {
    ...
    const initialHeight = Math.max(
      Math.min(window.innerHeight * 0.3, 300),
      200
    );
    ...
    const opts: uPlot.Options = {
      ...
      series: [
        {
          label: "Time (s)",
          value: (self: any, rawValue: number | null) =>
            rawValue == null ? "0.00" : rawValue.toFixed(2),
        },
  ...
  useEffect(() => {
    if (!uPlotRef.current || history.length === 0 || packetCount === null)
      return;
    ...
    const timestamps = recentHistory.map(
      (point) => (point.timestamp - latestTime) / 1000
    );
...
function EmgChart({ samplingRate }: { samplingRate: number }) {
  ...
  useEffect(
    () =>
      useEmgStore.subscribe((state) => {
        if (
          !uPlotRef.current ||
          history.length === 0 ||
          state.packetCount === null
        )
          return;

        // Find the most recent timestamp
        const latestTime = state.history[history.length - 1].timestamp;
...
function Hand({
  ...
  useEffect(() => {
    ...
    const filteredEmgUnsubscribe = myoMod.subscribeFilteredEmgData(
